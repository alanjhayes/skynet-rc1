...
class LightweightEmbeddingService:
    ...
    def fit_and_transform(self, texts: List[str]) -> np.ndarray:
        ...
        target_dim = self.vectorizer.max_features or 5000
        ...
        if embedding_array.shape[1] < target_dim:
            ...
            padded = np.zeros((embedding_array.shape[0], target_dim))
            padded[:, :embedding_array.shape[1]] = embedding_array
            return padded
        elif embedding_array.shape[1] > target_dim:
    ...
    def transform(self, texts: Union[str, List[str]]) -> np.ndarray:
        ...
        if not clean_texts:
            return np.zeros((len(texts), target_dim))
    ...
    def get_embedding_dimension(self) -> int:
        ...
        # Always return the target dimension (max_features)
